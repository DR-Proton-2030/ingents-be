"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.buildFacebookDashboardBuilder = void 0;
const page_service_1 = require("./page.service");
const content_service_1 = require("./content.service");
const insights_service_1 = require("./insights.service");
const audience_service_1 = require("./audience.service");
const traffic_service_1 = require("./traffic.service");
function isoDateDaysAgo(days) {
    return new Date(Date.now() - days * 24 * 60 * 60 * 1000)
        .toISOString()
        .split("T")[0];
}
function resolveWindow(preset) {
    const nowISO = new Date().toISOString().split("T")[0];
    switch (preset) {
        case "7d":
            return { sinceISO: isoDateDaysAgo(7), untilISO: nowISO };
        case "90d":
            return { sinceISO: isoDateDaysAgo(90), untilISO: nowISO };
        case "28d":
        default:
            return { sinceISO: isoDateDaysAgo(28), untilISO: nowISO };
    }
}
// Build a Facebook dashboard-style response using only supported Meta Graph API metrics
function buildFacebookDashboardBuilder(pageId_1, accessToken_1) {
    return __awaiter(this, arguments, void 0, function* (pageId, accessToken, preset = "28d") {
        const window = resolveWindow(preset);
        // Header: page profile
        const page = yield (0, page_service_1.getPageProfile)(pageId, accessToken);
        // Content: posts/videos in window
        const [posts, videos] = yield Promise.all([
            (0, content_service_1.fetchPostsInWindow)(pageId, accessToken, window, 50),
            (0, content_service_1.fetchVideosInWindow)(pageId, accessToken, window, 100),
        ]);
        const topContent48hWindow = {
            sinceISO: isoDateDaysAgo(2),
            untilISO: new Date().toISOString().split("T")[0],
        };
        const videos48h = yield (0, content_service_1.fetchVideosInWindow)(pageId, accessToken, topContent48hWindow, 50);
        const topContent48h = yield (0, content_service_1.getTopContent)(videos48h, accessToken, 5);
        // Overview totals
        const videoIds = videos.map((v) => v.id);
        const totals = yield (0, insights_service_1.getTotalsForWindow)(pageId, accessToken, videoIds);
        const followersDelta = yield (0, insights_service_1.getFollowersDelta)(pageId, accessToken, window);
        const viewsOverTime = yield (0, insights_service_1.getViewsOverTimeDaily)(pageId, accessToken, window);
        // Content section
        const publishedContent = [
            ...videos.map((v) => {
                var _a, _b, _c;
                return ({
                    id: v.id,
                    type: "video",
                    title: v.title || v.description || "",
                    permalink_url: v.permalink_url,
                    created_time: v.created_time,
                    thumbnail: (_c = (_b = (_a = v.thumbnails) === null || _a === void 0 ? void 0 : _a.data) === null || _b === void 0 ? void 0 : _b[0]) === null || _c === void 0 ? void 0 : _c.uri,
                });
            }),
            ...posts.map((p) => ({
                id: p.id,
                type: p.type || "post",
                title: p.message || "",
                permalink_url: p.permalink_url,
                created_time: p.created_time,
                thumbnail: p.full_picture,
            })),
        ];
        // Engagement totals via supported post fields; over-time breakdown not available as an official metric
        const engagementTotals = (0, content_service_1.sumPostEngagement)(posts);
        // Audience: followers vs non-followers and demographics
        const followerSplit = yield (0, audience_service_1.getFollowersVsNonFollowersViews)(videoIds, accessToken);
        const demographics = yield (0, audience_service_1.getDemographics)(pageId, accessToken);
        // Traffic sources: not available in Meta Graph API for pages
        const trafficSources = (0, traffic_service_1.getTrafficSources)();
        // Content Performance (Impressions): Not available as a full CTR/watch flow
        const impressionsUnavailable = true;
        return {
            page: page || {},
            overview: {
                views: totals.views,
                views3s: totals.views3s,
                views1m: totals.views1m,
                watchTime: totals.watchTime,
                threeSecondViewsUnavailable: totals.threeSecondViewsUnavailable,
                oneMinuteViewsUnavailable: totals.oneMinuteViewsUnavailable,
                followersGained: followersDelta.followersGained,
                followersLost: followersDelta.followersLost,
                netFollowers: followersDelta.netFollowers,
                viewsOverTime,
                topContent: topContent48h, // surface recent top to mirror Studio-like view
            },
            content: {
                totalViews: totals.views,
                publishedContent,
                viewerTypeUnavailable: true, // Meta does not provide new vs returning viewers segmentation
                viewerTypes: [],
            },
            engagement: {
                totals: engagementTotals,
                engagementOverTimeUnavailable: true, // No official daily engagement metric across posts
            },
            audience: Object.assign(Object.assign({ followers: (page === null || page === void 0 ? void 0 : page.fan_count) || 0, totalViews: totals.views }, followerSplit), { demographics }),
            trafficSources,
            impressions: {
                impressionsUnavailable,
                reason: "Not available via Meta Graph API",
                impressions: 0,
                ctr: 0,
                watchTimeFromImpressions: 0,
            },
        };
    });
}
exports.buildFacebookDashboardBuilder = buildFacebookDashboardBuilder;
